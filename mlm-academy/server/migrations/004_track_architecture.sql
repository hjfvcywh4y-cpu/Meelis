-- MLM Academy · маршрутная архитектура v2
-- Локальная/тестовая миграция. НЕ применять к production без отдельной команды.
-- Адаптация spec/track-architecture/backend-reference-schema.sql.
-- Запуск локально (пример):
--   psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f server/migrations/004_track_architecture.sql
-- Первичный runtime в этой итерации — in-memory store + CLI .local/, не KV.
